...
#hurt on low health
execute if score healthsound refresh_settings matches 1 as @a[scores={refresh_player_health=..5},advancements={vanilla_refresh:player/hurt=true}] at @s run function vanilla_refresh:player/health/hurt_low
...
#spectator actions
execute if score spectate refresh_settings matches 1 as @a[gamemode=spectator,tag=!refresh_spectator] at @s run function vanilla_refresh:player/spectator/enter
execute if score spectate refresh_settings matches 1 as @a[gamemode=!spectator,tag=refresh_spectator] at @s run function vanilla_refresh:player/spectator/exit
